...
export class ProblemStep {
    constructor() {
        ...
        this.hasPerformedAnalysis = false;
    ...
    setupEventListeners() {
        ...
        eventManager.on('workflow:step_changed', (data) => {
            if (data.currentStep === 'problem' && data.previousStep === 'field') {
                if (!this.state.analysisComplete && this.state.currentField && !this.hasPerformedAnalysis) {
    ...
    async load() {
        ...
        try {
            ...
            if (cachedResults) {
                ...
                this.hasPerformedAnalysis = true;
                this.render('results');
                ...
                if (this.state.analysisComplete && this.state.allProblems.length > 0) {
                    ...
                    this.hasPerformedAnalysis = true;
                    this.render('results');
                } else if (!this.hasPerformedAnalysis) {
                    // Auto-analyze for new field only if we haven't performed analysis yet
    ...
    loadCurrentState() {
        ...
        if (problemData) {
            ...
            // If we have analysis complete, mark that we've performed analysis
    ...
    async generateAIProblem() {
        ...
        console.log("Generating AI problem for field:", fieldId, "with metadata:", metadata, this.services.generationAdapter);
        try {
    ...
    renderResults() {
        ...
        // Always show Change Field button after first analysis
        const showChangeFieldButton = this.hasPerformedAnalysis || this.state.analysisComplete;
        console.log('AI Research Problem:', this.state.aiProblem);
        this.container.innerHTML = `
            <div class="step-container">
                <div class="step-header">
                    <h2 class="step-title">
                ...
                <div class="step-content">
                    <div class="ai-problem-section">
                        ...
                        <div class="ai-problem-card ${this.state.selectedProblem?.id === 'ai-generated' ? 'selected' : ''}" 
                            ...
                            <h4 class="problem-title editable-title" id="ai-problem-title" data-field="title">
                                ${this.escapeHtml(this.state.aiProblem?.title || 'Research Problem')}
                            </h4>
                            ...
                            <div class="problem-description-container">
                                <p class="problem-description editable-description" id="ai-problem-description" data-field="description">
                                    ${this.escapeHtml(this.state.aiProblem?.description || 'Click to edit problem description...')}
                                </p>
                            </div>
                            
                            <div class="problem-actions">
                                <button class="btn ${this.state.selectedProblem?.id === 'ai-generated' ? 'btn-success' : 'btn-primary'} select-problem-btn" 
                                        data-problem-id="ai-generated">
                                    <i class="fas ${this.state.selectedProblem?.id === 'ai-generated' ? 'fa-check-circle' : 'fa-check'}"></i>
                                    ${this.state.selectedProblem?.id === 'ai-generated' ? 'Selected' : 'Select This Problem'}
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    renderProblemsList(problems) {
        // Show ALL problems, not limited to 50
        const problemsToShow = problems;
            ...
            <div class="orkg-problems-list-section">
                <div class="section-header">
                    ...
                    <div class="results-info">
                        Showing ${problemsToShow.length} of ${problems.length} matched / ${this.state.allProblems.length} total scanned
                    </div>
                </div>
                
                <div class="problems-list">
                    ${problemsToShow.map((problem, index) => this.renderProblemCard(problem, index)).join('')}
                </div>
    ...
    renderProblemCard(problem, index) {
        ...
        const orkgUrl = problem.id ? `https://orkg.org/resource/${problem.id}` : null;
        ...
        return `
            <div class="problem-card ${isSelected ? 'selected' : ''}" 
                ...
                <div class="problem-header">
                    ...
                    <div class="problem-content">
                        ...
                        ${problem.id ? `
                            <div class="card-metadata">
                                ...
                                ${orkgUrl ? `
                                    <a href="${orkgUrl}" target="_blank" rel="noopener noreferrer" class="metadata-link">
                    ...
                    <div class="similarity-badge ${this.getSimilarityClass(similarity)}" style="flex-shrink: 0;">
                        <div class="similarity-value">${(similarity * 100).toFixed(1)}%</div>
                        <div class="similarity-bar">
                            <div class="similarity-fill ${this.getSimilarityClass(similarity)}" style="width: ${similarity * 100}%"></div>
                        </div>
                    </div>
                </div>
                ${isSelected ? `
    ...
    setupResultHandlers() {
        ...
        problemCards.forEach(card => {
            card.addEventListener('click', (e) => {
                // Don't select if clicking on inline editor or ORKG link
                if (e.target.classList.contains('inline-editor') || 
                    e.target.closest('.inline-editor') ||
                    e.target.closest('.metadata-link')) {
                    return;
                }
    ...
    updateProblemsList() {
        ...
        if (problemsList) {
            // Show ALL filtered problems, not limited
            const problemsToShow = filteredProblems;
            
            problemsList.style.opacity = '0.5';
            problemsList.style.transition = 'opacity 0.3s ease';
            
            setTimeout(() => {
                problemsList.innerHTML = problemsToShow.map((problem, index) => 
        ...
        if (resultsInfo) {
            resultsInfo.textContent = `Showing ${filteredProblems.length} of ${filteredProblems.length} matched / ${this.state.allProblems.length} total scanned`;
        }
    }
    ...
    attachProblemCardHandlers() {
        ...
        problemCards.forEach(card => {
            card.addEventListener('click', (e) => {
                // Don't select if clicking on ORKG link
                if (e.target.closest('.metadata-link')) {
                    return;
                }
    ...
    handleFieldChange(newField) {
        ...
        // Reset the hasPerformedAnalysis flag for new field
        this.hasPerformedAnalysis = false;
        ...
        if (cachedAnalysis) {
            // Mark that we have performed analysis if using cached results
            this.hasPerformedAnalysis = true;
            this.showFieldChangeOptions(cachedAnalysis);
        } else {
            this.resetAnalysisState();
            // Update state manager immediately so navigation works
            this.updateStateManager();
        }
    }
